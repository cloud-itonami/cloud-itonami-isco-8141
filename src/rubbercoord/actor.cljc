(ns rubbercoord.actor
  "RubberCoordActor — the ISCO-08 8141 rubber products machine
  ...
  The unconditional invariant: the Rubber Products Plant Scheduling
  Coordination Advisor can never directly commit an operating record
  the RubberCoordGovernor refuses, and can never make this actor
  ...
  area safety cleared), and can never override a plant safety
  ...
  curing equipment itself, and it never makes a plant-safety-clearance
  decision itself."
...
(defn build-graph
  "Build a compiled RubberCoordActor graph. `store` implements
  `rubbercoord.store/Store`. `advisor` implements
  `rubbercoord.advisor/Advisor` (defaults to `mock-advisor`).
