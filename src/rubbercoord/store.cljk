(ns rubbercoord.store
  "SSoT for the ISCO-08 8141 rubber products machine operators plant
  ...
  and raw-rubber/compound supply-order coordination for a rubber
  products machine operator crew under this advisor/governor pair,
  which never dispatches hardware itself, never operates molding/
  extrusion/curing equipment itself, and never finalizes a molding-
  operation-execution decision or a plant-safety-clearance decision,
  and never overrides a plant safety officer's judgment — those remain
  the plant safety officer's exclusive judgment). Modeled closely on
  cloud-itonami-isco-8122's platingcoord.store.

  Domain:

    operator — a registered rubber products machine operator crew
               member (:operator-id, :name)
    plant    — a registered rubber products molding/extrusion/curing
               plant or line {:plant-id :name :max-supply-cost
               number}. `:max-supply-cost` is an informational
               registered ceiling used only to decide whether a
    ...
    record   — a committed operating record (a logged production-run/
               ...
               raw-rubber/compound supply order) — written ONLY via
               commit-record!. This actor coordinates plant
               scheduling/logistics ONLY — a `record` is a
               coordination artifact, never a molding-operation-
