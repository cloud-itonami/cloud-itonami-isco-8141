(ns rubbercoord.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [rubbercoord.actor :as actor]
...
(deftest interrupts-then-approves-safety-concern-on-human-approval
  (let [st (fresh-store)
        ...
        request {:operator-id "operator-1" :op :flag-safety-concern :stake :low
                  :plant-id "P-1" :hazard-type :heat-exposure}
...
(deftest holds-a-scope-excluded-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a molding-operation-execution decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:operator-id "operator-1" :op :finalize-molding-decision :stake :low
                    :plant-id "P-1" :task "molding decision"}
